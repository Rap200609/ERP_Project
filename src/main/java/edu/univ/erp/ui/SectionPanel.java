...
public class SectionPanel extends JPanel {
    ...
    private final List<CourseDetail> courses = new ArrayList<>();
    private final ArrayList<Integer> courseIds = new ArrayList<>();
    ...
    private AdminSectionApi.SectionCommand buildCommandFromFields() {
        ...
        int sel = courseBox.getSelectedIndex();
        if (sel < 0 || sel >= courseIds.size()) {
            messageLabel.setText("Select a course.");
            return null;
        }
        command.courseId = courseIds.get(sel);
        command.sectionCode = sectionCodeField.getText().trim();
        ...
        if (command.sectionCode.isEmpty() || command.day.isEmpty() || command.time.isEmpty()
                || command.room.isEmpty() || command.semester.isEmpty() || yearStr.isEmpty() || capStr.isEmpty()) {
            messageLabel.setText("All fields required.");
            return null;
        }

        try {
            command.year = Integer.parseInt(yearStr);
            command.capacity = Integer.parseInt(capStr);
            if (command.capacity <= 0) {
                messageLabel.setText("Capacity must be > 0.");
                return null;
            }
    ...
    private void loadCourseList() {
        try {
            List<CourseDetail> apiCourses = sectionApi.listCourses();
            courses.clear();
            courseIds.clear();
            courseBox.removeAllItems();
            if (apiCourses != null) {
                courses.addAll(apiCourses);
                for (CourseDetail c : courses) {
                    courseBox.addItem(c.getCode() + " - " + c.getTitle());
                    courseIds.add(c.getCourseId());
                }
            }
        } catch (Exception ex) {
            // fallback: keep courseBox empty; optionally log ex
        }
    }
    ...
    private void editSectionDialog(int sectionId) {
        ...
        JTextField dayField = new JTextField(detail.getDay() != null ? detail.getDay() : "", 15);
        JTextField timeField = new JTextField(detail.getTime() != null ? detail.getTime() : "", 15);
        JTextField roomField = new JTextField(detail.getRoom() != null ? detail.getRoom() : "", 15);
        JTextField semesterField = new JTextField(detail.getSemester() != null ? detail.getSemester() : "", 15);
        JTextField yearField = new JTextField(String.valueOf(detail.getYear()), 15);
        JTextField capacityField = new JTextField(String.valueOf(detail.getCapacity()), 15);
        ...
        } else if (courseField.getItemCount() > 0) {
            courseField.setSelectedIndex(0);
        }

        // Add fields to dialog using helper pattern
        int row = 0;
        ...
        saveBtn.addActionListener(e -> {
            AdminSectionApi.SectionCommand command = new AdminSectionApi.SectionCommand();
            int selIdx = courseField.getSelectedIndex();
            if (selIdx < 0 || selIdx >= localCourseIds.size()) {
                infoLabel.setText("Select a course.");
                return;
            }
            command.courseId = localCourseIds.get(selIdx);
            ...
            try (Connection conn = edu.univ.erp.data.DatabaseConfig.getMainDataSource().getConnection();
                ...
                stmt.setInt(1, command.courseId);
                stmt.setString(2, command.sectionCode);
                stmt.setString(3, command.day);
                stmt.setString(4, command.time);
                stmt.setString(5, command.room);
                stmt.setString(6, command.semester);
                stmt.setInt(7, command.year);
                stmt.setInt(8, command.capacity);
                stmt.setInt(9, sectionId);
                stmt.executeUpdate();
                infoLabel.setText("Section updated!");
                loadSectionTable();
            } catch (Exception ex) {
                infoLabel.setText("Update failed: " + ex.getMessage());
            }
        });
    ...
    private Integer resolveSelectedCourseId(int index) {
        if (index < 0 || index >= courseIds.size()) {
            return null;
        }
